...
void update() {
    ...
    for(int i = 0; i < num_particles; i++) {
        for(int j = i + 1; j < num_particles; j++) {
            ...
            _circles[i].addAcceleration( xDif * _circles[j]._mass,  yDif * _circles[j]._mass);
            _circles[j].addAcceleration(-xDif * _circles[i]._mass, -yDif * _circles[i]._mass);
        }
    }
