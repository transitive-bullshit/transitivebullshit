...
;(function (window) {
  ...
  BoxesFx.prototype._init = function () {
    ...
    this._setTransforms();
    // which effect
    this.effect = ($el.hasClass('effect-1') && 'effect-1') || ($el.hasClass('effect-2') && 'effect-2') || ($el.hasClass('effect-3') && 'effect-3') || 'effect-2';
    // check if animating
    this.isAnimating = false;
    // the panels
    this.panels = [].slice.call(this.el.querySelectorAll('.box-panel'));
    // total number of panels
    this.panelsCount = this.panels.length;
    // total number of tiles per panel
    this.tilesCount = 4;
    // current panel´s index
    this.current = 0;
    $(this.panels[0]).addClass('current');
    // replace panel img with 4 divs, each including the img
    var self = this;
    this.panels.forEach(function (panel) {
      var img = panel.querySelector('img'), imgReplacement = '';
      for (var i = 0; i < self.tilesCount; ++i) {
  ...
  BoxesFx.prototype.navigate = function (dir, effect) {
    ...
    var cntTransTotal = 0,
      ...
      onEndTransitionFn = function (ev) {
        if (ev && !$(ev.target).hasClass('bg-img')) return false;

        // return if not all panel transitions ended
        ++cntTransTotal;
        if (cntTransTotal < self.tilesCount) return false;
